...
def plot_histogram_2(similarity_top):
    num_bins = 50
...
def og_loss_by_visual_text_contrast(similarity_visual, similarity_text, batch_mask, tau=1):
...
def loss_unet(similarity_visual, similarity_text, batch_indexes, tau=1):

    prob_visual = ...
    prob_text = ...

    prob_gt = prob_text[batch_indexes]
    prob_pred = prob_visual[batch_indexes]

    a = tf.maximum((prob_gt * tf.math.log(prob_gt / prob_pred)), 0)
    b = tf.maximum(((1 - prob_gt) * tf.math.log((1 - prob_gt) / (1 - prob_pred))), 0)

    loss = a + b

    loss_aggregate = tf.reduce_mean(loss)
