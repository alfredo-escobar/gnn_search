...
# def NUMPY_get_k_random_pairs(similarity, k = 100):
...
#     # k random 2D indexes for the similarity matrix:
#     random_idx_pairs = np.zeros((size*2,2), dtype='int32')
#     random_idx_pairs[:size, 0] = similarity_idx[0][random_indexes_similar]
#     random_idx_pairs[:size, 1] = similarity_idx[1][random_indexes_similar]

#     random_idx_pairs[size:, 0] = similarity_idx[0][random_indexes_dissimilar]
#     random_idx_pairs[size:, 1] = similarity_idx[1][random_indexes_dissimilar]

#     return random_idx_pairs

    # batch_mask = np.zeros((similarity.shape[0], similarity.shape[0]), dtype='float32')
    # batch_mask[similarity_idx[0][random_indexes_similar],    similarity_idx[1][random_indexes_similar]] = 1
    # batch_mask[similarity_idx[0][random_indexes_dissimilar], similarity_idx[1][random_indexes_dissimilar]] = 1
...
def get_similarity_tensor(embeddings, tau=1):
...
def loss_by_visual_text_contrast(similarity_visual, similarity_text):#, batch_mask):
    ...
    loss_aggregate = tf.reduce_sum(loss_for_each_pair)

    return loss_aggregate


# def loss_by_visual_text_contrast(similarity_visual, similarity_text, pairs_batch):
...
#     loss_aggregate = tf.Variable(0, dtype=tf.float32)

#     for i,j in pairs_batch:
#         loss_aggregate = tf.add(loss_aggregate, loss_for_each_pair[i,j])

#     return loss_aggregate


def train_visual(visual_embeddings, text_embeddings, units, iterations, lr, mAP_dictionary = None):
    ...
    for op in range(iterations + 1):
        ...
        #batch_mask = get_k_random_pairs(similarity=similarity_text.numpy(), k=100)
        ...
        with tf.GradientTape() as t:
            ...
            # loss = loss_by_visual_text_contrast(curr_similarity_text, curr_similarity_visual, batch_mask)
            loss = loss_by_visual_text_contrast(similarity_text, similarity_visual)
...
if __name__ == '__main__' :
    ...
    if pargs.mode == 'gnn':
        ...
        if not train_visuals:
            ...
            new_visual_embeddings, historical_loss, historical_mAP = train_visual(visual_embeddings, text_embeddings, None, iterations, 0.01, mAP_dictionary)

            iters_axis = ["Original", *range(iterations + 1)]
            fig = plt.figure()

            # Top subplot (2 rows, 1 col, subplot 1)
            ax1 = fig.add_subplot(2, 1, 1)
            ax1.plot(iters_axis, historical_loss)
            ...
            ax2.plot(iters_axis, historical_mAP)
